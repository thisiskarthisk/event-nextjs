...
export default function UserAddOrEditForm({ params }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      // If auto-generated, ensure employee_id is null/undefined in data submitted
      const submitData = { 
        id: decryptedUserId, 
        ...formData 
      };
      ...
      HttpClient({
        ...
        const message = err.response?.message || 'Error occurred when trying to save the User data.';
        
        // Handle specific auto-generation error (if backend returns it)
        if (message.includes("Employee ID Set in Setting")) {
             // The backend handles this error if the field is empty, but we rely on the FE state check for the alert.
             toast('error', message);
        } else {
